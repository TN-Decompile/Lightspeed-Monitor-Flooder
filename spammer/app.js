...
var config=JSON.parse(fs.readFileSync('config.json','utf8')),
	...
	genInfo=(()=>{
		...
		info.udid='xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,(c)=>{var r = Math.random() * 16 | 0, v = c == 'x' ? r : (r & 0x3 | 0x8);return v.toString(16)});
		info.email=names[Math.floor(Math.random()*names.length)]+'@'+domains[Math.floor(Math.random()*domains.length)];
		info.gps=[(Math.random() * (180 - -180) + -180).toFixed(3)*1,(Math.random() * (180 - -180) + -180).toFixed(3)*1];
		return info;
	});
